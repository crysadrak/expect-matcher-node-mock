...
function toHaveBeenCalled(receivedMethod, ...args) {
  ...
  if (args.length > 0) {
    return {
      pass: false,
      message: () => `${hint}Matcher error: this matcher must not have an expected argument\n` +
...
function toHaveBeenLastCalledWith(receivedMethod, ...args) {
  ...
  const receivedArgs = lastCall.arguments.map(arg => printReceived(arg)).join(', ');
...
function toHaveBeenNthCalledWith(receivedMethod, nthCallIndex, ...args) {
  ...
  const receivedArgs = nthCall.arguments.map(arg => printReceived(arg)).join(', ');
